...
df = df[
    (df["Orphan_status"] == 1) &
    (df["Emergence"].isin(["diverged", "denovo"]))
]

df = df.drop(columns=[
    "Species", "Orphan_status", "InterPro", "SignalP", "Number_of_homologs", 
    "Orthogroup", "Root", "Transcription", "Proteomics"
])
df = df.set_index("Sequence_id")

df["Emergence"] = df["Emergence"].map({"denovo": 1, "diverged": 0})

X = df.drop(columns=["Emergence"])
y = df["Emergence"].values
...
with open("rf_denovo_best.pkl", "wb") as f:
    pickle.dump(final_model, f)

dump(final_model, "rf_denovo_best.joblib")

print("\nModel saved as rf_orphan_best.pkl and rf_denovo_best.joblib")
